    ...
    ${config.name}<span class="token punctuation">.</span>projects <span class="token operator">=</span> <span class="token punctuation">[</span>
      ${config.projects.reduce((str, p, i) => {
        if (i + 1 < config.projects.length) {
          var substr = `<span class="token string">"<a class="token md-link" href="${p[1]}">${p[0]}</a>"</span><span class="token punctuation">,</span>\n`
        } else {
          var substr = `<span class="token string">"<a class="token md-link" href="${p[1]}">${p[0]}</a>"</span>`
        }
        return str + substr
      }, ``)}
    <span class="token punctuation">]</span>
    ...
    ${config.name}<span class="token punctuation">.</span>articles <span class="token operator">=</span> <span class="token punctuation">[</span>
      ${site.files.reduce((str, p, i) => {
        ...
        if (i + 1 < site.files.length) {
          var substr = `<span class="token string">"<a class="token md-link" href="./#articles/${p.history[2].replace('.html', '')}">${p.metadata.title}</a>"</span><span class="token punctuation">,</span>\n`
        } else {
          var substr = `<span class="token string">"<a class="token md-link" href="./#articles/${p.history[2].replace('.html', '')}">${p.metadata.title}</a>"</span>`
        }
